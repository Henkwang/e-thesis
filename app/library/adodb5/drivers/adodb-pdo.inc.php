...
class ADODB_pdo extends ADOConnection {
	...
	function _connect($argDSN, $argUsername, $argPassword, $argDatabasename, $persist=false)
	{
		...
		if ($argDatabasename) {
			$dbname = 'dbname';
			if($this->dsnType=='sqlsrv')$dbname = 'database';
			$argDSN .= ';'.$dbname.'='.$argDatabasename;
			if(is_string($this->charSet)) $argDSN .=';charset='.$this->charSet;
		}
		try {
	...
	{
		$args = func_get_args();
		if(method_exists($this->_driver, 'ToSQLDate')) 
			return call_user_func_array(array($this->_driver, 'ToSQLDate'), $args); 
		
		return call_user_func_array(array($this,'parent::ToSQLDate'), $args); 
	}

	function ifNull() 
	{
		$args = func_get_args();
		if(method_exists($this->_driver, 'ifNull')) 
			return call_user_func_array(array($this->_driver, 'ifNull'), $args); 
		
		return call_user_func_array(array($this,'parent::ifNull'), $args); 
	}
	...
	{	
		...
		if($this->dsnType!="sqlsrv")$this->_connectionID->setAttribute(PDO::ATTR_AUTOCOMMIT,false);
	...
	{ 
		...
		if($this->dsnType!="sqlsrv")$this->_connectionID->setAttribute(PDO::ATTR_AUTOCOMMIT,true);
		return $ret;
	}
