...
class TCPDF  {
	...
	public function TCPDF($orientation='P', $unit='mm', $format='A4', $unicode=true, $encoding='UTF-8', $diskcache=false, $pdfa=true) {
//                if(preg_match("/Firefox/",$_SERVER['HTTP_USER_AGENT']) || strstr($SERVER['HTTPUSERAGENT'],'iPhone') || strstr($SERVER['HTTPUSERAGENT'],'iPod')  || strstr($SERVER['HTTPUSERAGENT'],'iPad')){  $pdfa = true;  }
...
public function AddFont($family, $style='', $fontfile='', $subset='default') {
		...
		$missing_style = false; 				if (TCPDF_STATIC::empty_string($fontfile) OR (!@file_exists($fontfile))) {
...
public function Write($h, $txt, $link='', $fill=false, $align='', $ln=false, $stretch=0, $firstline=false, $firstblock=false, $maxh=0, $wadj=0, $margin='') {
		...
		$i = 0; 		$j = 0; 		$sep = -1; 		$shy = false; 		$l = 0; 		$nl = 0; 		$linebreak = false;
		$pc = 0; 				while ($i < $nb) {
		...
		$exurl = ''; 				if ($file[0] === '@') {
			...
			if ($imsize === FALSE) {
				if (function_exists('curl_init')) {
										$cs = curl_init(); 					curl_setopt($cs, CURLOPT_URL, $file);
...
protected function ImagePngAlpha($file, $x, $y, $wpx, $hpx, $w, $h, $type, $link, $align, $resize, $dpi, $palign, $filehash='') {
		if (empty($filehash)) {
			$filehash = md5($this->file_id.$file);
				...
				$tempfile_plain = K_PATH_CACHE.'mskp_'.$filehash;
				$tempfile_alpha = K_PATH_CACHE.'mska_'.$filehash;
		if (extension_loaded('imagick')) { 						$img = new Imagick();
			$img->readImage($file);
		...
		} elseif (function_exists('imagecreatefrompng')) { 						$img = imagecreatefrompng($file);
			$imgalpha = imagecreate($wpx, $hpx);
				...
				for ($ypx = 0; $ypx < $hpx; ++$ypx) {
					...
					$alpha = $this->getGDgamma($color); 					imagesetpixel($imgalpha, $xpx, $ypx, $alpha);
				}
			}
...
protected function _enddoc() {
		...
		$this->_out($o);
		$this->_out('%%EOF');
		$this->state = 3; 		if ($this->diskcache) {
...
protected function _generateencryptionkey() {
		...
		if (!$this->encryptdata['pubkey']) { 			if ($this->encryptdata['mode'] == 3) { 								$this->encryptdata['key'] = substr(hash('sha256', TCPDF_STATIC::getRandomSeed(), true), 0, $keybytelen);
				...
				$perms .= 'adb'; 				$perms .= 'nick'; 				$iv = str_repeat("\x00", mcrypt_get_iv_size(MCRYPT_RIJNDAEL_128, MCRYPT_MODE_ECB));
...
public function SetProtection($permissions=array('print', 'modify', 'copy', 'annot-forms', 'fill-forms', 'extract', 'assemble', 'print-high'), $user_pass='', $owner_pass=null, $mode=0, $pubkeys=null) {
		...
		if ($mode > 1) { 			if (!extension_loaded('mcrypt')) {
...
public function Button($name, $w, $h, $caption, $action, $prop=array(), $opt=array(), $x='', $y='', $js=false) {
	...
	protected function _putsignature() {
		...
		$out .= ' >>'; 												$out .= ' >>';
		$out .= ' ]'; 		if (isset($this->signature_data['info']['Name']) AND !TCPDF_STATIC::empty_string($this->signature_data['info']['Name'])) {
...
public function colorRegistrationBar($x, $y, $w, $h, $transition=true, $vertical=false, $colors='A,R,G,B,C,M,Y,K') {
		$bars = explode(',', $colors);
		$numbars = count($bars); 				if ($vertical) {
...
public function Gradient($type, $coords, $stops, $background=array(), $antialias=false) {
		...
		switch($numcolspace) {
			case 4: { 				$this->gradients[$n]['colspace'] = 'DeviceCMYK';
				if (!empty($background)) {
		...
		foreach ($stops as $key => $stop) {
			...
			switch($numcolspace) {
				case 4: { 					$this->gradients[$n]['colors'][$key]['color'] = sprintf('%F %F %F %F', $color[0]/100, $color[1]/100, $color[2]/100, $color[3]/100);
					break;
				}
...
public function write1DBarcode($code, $type, $x='', $y='', $w='', $h='', $xres='', $style='', $align='') {
		...
		$this->SetTextColorArray($style['fgcolor']);
			...
			$cellpadding = $this->cell_padding;
                        $aligntext = empty($style['aligntext']) ? 'C' :  $style['aligntext'];
			$this->SetCellPadding(0);
			$this->Cell($txtwidth, '', $label, 0, 0, $aligntext, false, '', $style['stretchtext'], false, 'T', 'T');
			$this->cell_padding = $cellpadding;
		}
				$this->rtl = $tempRTL;
				$this->setGraphicVars($gvars);
...
public function hyphenateText($text, $patterns, $dictionary=array(), $leftmin=1, $rightmin=2, $charmin=1, $charmax=8) {
		...
		$word = array(); 		$txtarr = array(); 		$intag = false; 		if (!is_array($patterns)) {
...
protected function startSVGElementHandler($parser, $name, $attribs, $ctm=array()) {
		...
		if ($parser == 'clip-path') {
				...
				$prev_svgstyle = $this->svgstyles[(count($this->svgstyles) - 1)]; 		$svgstyle = $this->svgstyles[0]; 		if ($clipping AND !isset($attribs['fill']) AND (!isset($attribs['style']) OR (!preg_match('/[;\"\s]{1}fill[\s]*:[\s]*([^;\"]*)/si', $attribs['style'], $attrval)))) {
						$attribs['fill'] = 'none';
		}
...
protected function segSVGContentHandler($parser, $data) {
    ...
    /* @ToNict : add-on */
    protected $boldLineWidth = 0.3;

    /* @ToNict  : กำหนดระยะตัวหนังสือกับกรอบด้านใน */
    public function setCellPaddingRight($right) {
        if (($right !== '') && ($right >= 0)) {
            $this->cell_padding['R'] = $right;
        }
    }
    ...
    /* @ToNict  : กำหนดระยะตัวหนังสือกับกรอบด้านใน */
    public function setCellPaddingLeft($left) {
        if (($left !== '') && ($left >= 0)) {
            $this->cell_padding['L'] = $left;
        }
    }
    ...
    /* @ToNict  : กำหนดระยะตัวหนังสือกับกรอบด้านใน */
    public function setCellPaddingTop($top) {
        if (($top !== '') && ($top >= 0)) {
            $this->cell_padding['T'] = $top;
        }
    }
    ...
    /* @ToNict  : กำหนดระยะตัวหนังสือกับกรอบด้านใน */
    public function setCellPaddingBottom($bottom) {
        if (($bottom !== '') && ($bottom >= 0)) {
            $this->cell_padding['B'] = $bottom;
        }
    }

    /* @ToNict  : เส้นขอบหนา ตามที่กำหนด นอกหนือจากที่กำหนดให้ใช้ค่าความหนาจาก class */
    public function BorderBold($border) {
        $notBorder = '';
        ...
        return array($border => array('width' => $this->boldLineWidth), $notBorder => array('width' => $this->LineWidth));
    }
