...
class Hrd_division_model extends \EThesis\Library\Adodb
{

    var $schema = 'hrd';
    var $table = 'HRD_MAS_DIVISION';
    var $primary = 'DIVISION_ID';
...
//        $this->adodb->debug = TRUE;
    ...
    {
        $sql = "RECORD_STATUS='N' AND DIVISION_NAME_TH IS NOT NULL ";
        if (empty($filter)) {

        } else if (is_array($filter)) {
            $sql .= (isset($filter['DIVISION_CODE']) ? " AND DIVISION_CODE='{$filter['DIVISION_CODE']}'" : '');
            $sql .= (isset($filter['DIVISION_NAME_TH']) ? " AND DIVISION_NAME_TH LIKE'%{$filter['DIVISION_NAME_TH']}%'" : '');
            $sql .= (isset($filter['DIVISION_NAME_EN']) ? " AND DIVISION_NAME_EN='{$filter['DIVISION_NAME_EN']}'" : '');
